...
The release includes the release artifact `install` which can be used to install the `botctrl` management system. See the [Install botctrl](#install-botctrl) section below for installation instructions. See the [repository README](https://github.com/slbotcontrol/BotControl#readme) for additional info and example `botctrl` command invocations.
